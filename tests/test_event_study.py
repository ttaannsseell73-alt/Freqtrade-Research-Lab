...
def test_short_return_uses_linear_futures_pnl() -> None:
    frame = synthetic_frame()
    train_end = frame["date"].iloc[-1] + pd.Timedelta(days=1)
    validation_end = train_end + pd.Timedelta(days=1)
    config = EventStudyConfig(horizons=(1,), round_trip_cost_bps=0)
    result = analyze_pair(frame, "TEST/USDT:USDT", config, train_end, validation_end)
    ...
    short_signal = (macd < signal) & (macd.shift(1) >= signal.shift(1)) & (frame["volume"] > 0)
    entry = frame["open"].shift(-1)
    exit_price = frame["close"].shift(-1)
    valid = short_signal & entry.notna() & exit_price.notna()
    expected = ((entry[valid] - exit_price[valid]) / entry[valid]).mean()

    actual = result.loc[
        (result["direction"] == "short")
        & (result["period"] == "train")
        & (result["horizon_minutes"] == 1),
        "mean_net_return",
    ].iloc[0]
